package com.imranmelikov.qrcodescankotlin
...
import com.imranmelikov.qrcodescankotlin.analyzer.MLKitBarcodeAnalyzer
import com.imranmelikov.qrcodescankotlin.analyzer.ScanningResultListener
import com.imranmelikov.qrcodescankotlin.analyzer.ZXingBarcodeAnalyzer
import com.imranmelikov.qrcodescankotlin.databinding.ActivityBarcodeScanningBinding
        ...
        class ScanningListener : ScanningResultListener {
            override fun onScanned(result: String) {
                runOnUiThread {
                    ...
                    cameraProvider?.unbindAll()
                    ScannerResultDialog.newInstance(result, object : ScannerResultDialog.DialogDismissListener {
                            override fun onDismiss() {
                                bindPreview(cameraProvider)
                            }
                        }).show(supportFragmentManager, ScannerResultDialog::class.java.simpleName)
                    Toast.makeText(this@BarcodeScanningActivity,"Qr code successed",Toast.LENGTH_SHORT).show()
                }
            }

            override fun onScannedWebsite(result: String) {
                runOnUiThread {
                    imageAnalysis.clearAnalyzer()
                    cameraProvider?.unbindAll()
                    ScannerResultDialog.newInstance(
                        ...
                        .show(supportFragmentManager, ScannerResultDialog::class.java.simpleName)
                    Toast.makeText(this@BarcodeScanningActivity,"Qr code website successed",Toast.LENGTH_SHORT).show()
                }
            }
        }
